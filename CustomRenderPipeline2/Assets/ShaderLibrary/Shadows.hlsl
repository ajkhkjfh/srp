...
float FadedShadowsStrength(float distance,float scale,float fade)
{
...
ShadowData GetShadowData(Surface surfaceWS)
{
    ...
    data.strength = FadedShadowsStrength(surfaceWS.depth,
    ...
    for (i = 0; i < _CascadeCount; i++) {
        ...
        if (distanceSqr < sphere.w) {
            float fade = FadedShadowsStrength(distanceSqr,_CascadeData[i].x,_ShadowDistanceFade.z);
